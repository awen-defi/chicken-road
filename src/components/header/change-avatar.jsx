...
export function ChangeAvatarModal({ ref }) {
  const [selectedAvatarIndex, setSelectedAvatarIndex] = useState(0);

  useEffect(() => {
    const savedIndex = settingsManager.get("selectedAvatarIndex");
    if (savedIndex !== undefined && savedIndex !== null) {
      // eslint-disable-next-line react-hooks/set-state-in-effect
      setSelectedAvatarIndex(savedIndex);
    }
  }, []);
  ...
  const handleSaveAndClose = () => {
    settingsManager.set("selectedAvatarIndex", selectedAvatarIndex);
    closeModal();
  };
  ...
  return (
    <dialog
      ref={ref}
      className="generic-dialog change-avatar-modal"
      onClick={(e) => e.stopPropagation()}
    >
      <div className="generic-dialog-header">
        <h2 className="generic-dialog-header-text">Change avatar</h2>
        <svg
          width="16"
          height="16"
          viewBox="0 0 16 16"
      ...
      <div data-testid="modal-avatar-option">
        <div className="AvatarList">
          {AVATARS.map((avatar, index) => (
            <div key={index}>
              <div
                className={`AvatarStyles ${
                  selectedAvatarIndex === index ? "Active" : ""
                }`}
        ...
        <div>
          <button className="Button" onClick={handleSaveAndClose}>
            Save and close
          </button>
        </div>
      </div>
